...
export function GroupBlock({ node, runtime }: GroupBlockProps) {
  ...
  return (
    <section className="grid gap-3" aria-label="Group node">
      <div className="flex items-center justify-between">
        <strong className="text-[0.84rem] text-foreground">{node.data.title}</strong>
        <span className="text-[0.66rem] font-extrabold text-foreground-tertiary">{node.data.nodeType === 'flow_group' ? 'Executable scope' : 'Organization'}</span>
      </div>
