...
27. Render managed object-storage media through the shared `MediaImage`, `MediaVideo`, and `apps/web/src/lib/media-url.ts` helpers. Frontend DTOs and component state must not store or render direct presigned storage URLs or session-token query params; use stable API content endpoints, scoped HttpOnly cookie authentication, browser-managed private caching, and component-level refresh retries only as a load-failure fallback for images, video posters, and playable video. Do not add session tokens to browser URLs; WebSocket authentication should use the session cookie in browser clients or `Sec-WebSocket-Protocol` for non-browser clients, and any browser Cookie-authenticated WebSocket endpoint must enforce the configured `MINA_ALLOWED_ORIGIN`. Ordinary JSON API routes should keep using bearer tokens rather than ambient cookie authentication; `/api/auth/logout` is the narrow exception because it must revoke the current HttpOnly browser session after local client state has already been cleared.
