...
## Specification
- Editable workflow graph data is stored in Yjs update logs and compacted snapshots. `workflows` stores metadata only; public detail DTOs still expose React Flow-compatible `nodes` and `edges` by exporting the current Yjs document.
...
- Workflow canvas graph edits use Yjs as the single source of truth. Zustand keeps UI/read projection state such as `nodes`, `edges`, selection, panels, and render cache; graph mutation actions are thin commands that write the live Yjs document.
...
- Workflow canvas visible-element rendering is controlled by a weighted performance policy instead of a raw node-count threshold. The policy scores nodes, edges, and media-generation nodes so media-heavy canvases enable React Flow visible-element culling earlier while small canvases avoid culling overhead.
- Media-generation node task detail queries are gated by node visibility plus a viewport margin. Off-screen nodes keep their cached preview state and do not start or refetch task-detail requests until they are near the viewport.
...
- Canvas diagnostics expose development counters for node changes, edge changes, render writes, document commits, websocket reconnects, Yjs update traffic, and React Profiler commits. Performance fixtures cover 20, 100, and 500-node canvases, with Chromium traces stored under the workflow-canvas performance refactor history package.
...
- The frontend no longer exposes editable graph autosave or manual save. It displays Yjs sync state (`Synced`, `Syncing`, `Offline`) while graph durability is handled by the authenticated WebSocket update path and server-side compaction.
- The API exposes `GET /api/workflows/:id/collab/snapshot` for diagnostics/initialization and `WS /api/workflows/:id/collab/:room` as the live sync channel. The WebSocket route authenticates and authorizes workflow access, enforces room/workflow id matching, persists Yjs updates before broadcast, broadcasts to peers without echoing the sender, compacts update streams into snapshots, and restores room state from persisted snapshots/updates after restart.
- Client graph commands perform a local dry-run plus graph validation before mutating the live ydoc. The server does not clone and validate the full graph on every Yjs update; it validates exported graphs during snapshot initialization and compaction, so high-frequency collaboration remains bounded while invalid graphs cannot become durable run inputs.
- High-frequency Yjs node-frame commits use incremental validation for node existence, finite positions, positive dimensions, and valid parent identifiers before writing the live ydoc. Structural graph changes still use full dry-run graph validation.
- Client Yjs projection comparison uses a cached snapshot signature for the current runtime to avoid repeated full parse/stringify validation on the update hot path. Development diagnostics still expose full snapshot parity checks.
- Yjs update application and compaction are serialized per workflow id. Workflow run creation compacts the authoritative server ydoc so execution snapshots consume the latest collaborative graph.
...
- Workflow canvas CSS avoids expensive active-canvas effects such as `backdrop-filter`, SVG `drop-shadow` filters, keyframe pulse/flow animations, and transition-driven scale/transform effects. The visual language is preserved with static color, border, and shadow tokens.
...
- `bun test apps/web/src/features/workflow-canvas/render/flow-performance-policy.spec.ts`
...
## Change History
- `helloagents/history/2026-05/202605210816_workflow_canvas_perf_policy/` - Added weighted React Flow rendering policy, viewport-gated media node task queries, Yjs node-frame hot-path validation/signature optimization, GPU-costly canvas CSS cleanup, direct source imports for registry helpers, and admin-only public-library media upload policy.
- `docs/design/workflow-yjs-ssot-storage-refactor.md` - Removed frontend autosave/checkpoint and editable SQL read-model storage so Yjs update logs/snapshots are the editable graph SSOT.
- `helloagents/history/2026-05/202605201040_workflow_canvas_yjs_ssot/` - Replaced workflow canvas shadow/full-save collaboration with Yjs SSOT graph commands and one-way projection. Its checkpoint/read-model notes are superseded by `docs/design/workflow-yjs-ssot-storage-refactor.md`.
